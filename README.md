...
Вертикальная полоса (LED) использует красный (#ff3b30) цвет как единственный цветной элемент; остальной контент монохром.

### Экран 5: Нет сети
![Экран 5](./images/screen5-no-internet.svg)

Вертикальная полоса (LED) использует жёлтый (#ffcc00); контент остаётся монохромным.
