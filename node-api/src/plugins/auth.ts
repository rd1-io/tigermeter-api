...
export default fp(async (app) => {
  app.decorate('requireUser', async (request: FastifyRequest) => {
    ...
    if (!auth?.startsWith('Bearer ')) {
      throw (app as any).httpErrors.unauthorized('Missing token');
    }
    const token = auth.slice('Bearer '.length);
    try {
      const payload = (await verifyJwt(token)) as UserToken;
      if (!payload.role || (payload.role !== 'user' && payload.role !== 'admin')) {
        throw (app as any).httpErrors.forbidden('Forbidden');
      }
      return payload;
    } catch {
      throw (app as any).httpErrors.unauthorized('Invalid token');
    }
  });
  ...
  app.decorate('requireAdmin', async (request: FastifyRequest) => {
    ...
    if (payload.role !== 'admin') {
      throw (app as any).httpErrors.forbidden('Admin only');
    }
    return payload;
  });
});
